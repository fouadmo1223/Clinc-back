export default () => {
  ...
  // In prod, a missing PATIENT_JWT_SECRET would silently reuse the staff JWT secret,
  // making patient and staff tokens cryptographically interchangeable — fail fast instead.
  if (nodeEnv === 'production' && !process.env.PATIENT_JWT_SECRET) {
